...
python create_uboot_boot_txt() {
    ...
    cfile = d.getVar('UBOOT_ENV_CONFIG', expand=True)
    if not cfile:
    ...
    try:
        with open(cfile, 'w') as cfgfile:
            ...
            kernel_devicetree = localdata.getVar('KERNEL_DEVICETREE', True)
            if kernel_devicetree is not None:
              fdtfile = os.path.basename(kernel_devicetree)
            ...
            bootextraargs = localdata.getVar('BOOT_EXTRA_ARGUMENTS', True)

            cfgfile.write('setenv bootargs \"${console} root=/dev/mmcblk${mmcrootdev}p${mmcrootpart} rw rootwait %s\"\n' % bootextraargs)
